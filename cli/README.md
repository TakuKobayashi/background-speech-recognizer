...
**文字起こしに成功してテキストが残ったセグメントだけ**、`.wav` と `.txt` のペアが `./outputs/` (または `--output` で指定したディレクトリ) に保存されます。

```
outputs/
├── 20260526_142301_0.wav   # 録音された音声 (発話のみ)
├── 20260526_142301_0.txt   # 文字起こし結果 (発話のみ)
...
| 録音セグメントの中身 | whisper の出力 | 保存される? |
|---|---|---|
| 普通の発話              | `今日は晴れです`            | ✅ wav + txt 両方保存 |
| 発話 + 効果音注釈       | `今日は晴れです (笑)`        | ✅ `今日は晴れです` だけが txt に保存 |
| 効果音 / BGM のみ       | `(音楽)` / `[Music]` / `♪`  | ❌ 何も保存しない (発話なしと判定) |
| 雑音のみ                | `[BLANK_AUDIO]` 等          | ❌ 何も保存しない |
| whisper エラー          | (空または error)             | ❌ 何も保存しない |
